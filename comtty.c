...
#define INPUT_BUF_SIZE 4096
  ...
  while (!ctrl_info->sig_term) {
    ...
    if (len > 1)
      fake_key = gen_fake_key(input_buf);
    switch (fake_key) {
    case KEYF1:
    case KEYF2:
    case KEYF3:
    ...
    case 0x12: // Ctrl+R
      ...
      }
      {
        ...
        case 4:
          ...
          break;
        case 5:
          ret = reload_config(gs_config_path, config_g, MAX_CONFIG_SZ);
...
void get_ttywinSize(void) { ioctl(fileno(stdin), TIOCGWINSZ, &g_tty_winsz); }
...
void check_report_screensize(char c, int dev) {
  ...
  if (esc_seq_idx > 0) {
    ...
    case 3:
      if (c == 'n') {
        char sendbuf[64];
        sprintf(sendbuf, "\x1b[%d;%dR", g_tty_winsz.ws_row, g_tty_winsz.ws_col);
...
void *term_output(void *data) {
  CTRL_INFO_t *ctrl_info = (CTRL_INFO_t *)data;
  int flags = fcntl(ctrl_info->term_pipe_fd, F_GETFL);
  fcntl(ctrl_info->term_pipe_fd, F_SETFL, flags | O_NONBLOCK);
  while (!ctrl_info->sig_term) {
    char c = 0;
    if (ctrl_info->sig_blockoutput == 0) {
      while (read(ctrl_info->term_pipe_fd, &c, 1) > 0) {
        write(STDOUT_FILENO, &c, 1);
      }
    }
    usleep(100);
  }
  close(ctrl_info->term_pipe_fd);
  return NULL;
}

void *port_reader(void *data) {
  CTRL_INFO_t *ctrl_info = (CTRL_INFO_t *)data;
  int device = ctrl_info->device;
  ...
  pthread_t tid[2];
  ctrl_info->term_pipe_fd = buf_fds[0];
  ctrl_info->log_pipe_fd = log_fds[0];

  pthread_create(&tid[0], NULL, term_output, ctrl_info);
  pthread_create(&tid[1], NULL, log_to_file, ctrl_info);
  while (!ctrl_info->sig_term) {
    char c = 0;
...
struct option options[] = {{"generate", no_argument, 0, 'g'},
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "hgc:p:b:d:s:r:l:", options,
                            &opt_idx)) != -1) {
    switch (opt) {
    case 'g':
      gen_config = 1;
  ...
  if (cfgfn == NULL) {
    ...
    sprintf(gs_config_path, "%s", CONFIG_FN);
#else
  ...
  if (load_config(gs_config_path, config_g, MAX_CONFIG_SZ) == -1) {
    if (gen_config) {
      if (-1 ==
          config_create_default(gs_config_path, config_g, MAX_CONFIG_SZ)) {
  ...
  if (config.parity == -1 &&
      (val = config_getvalue("Parity", config_g)) != NULL) {
    config.parity = (int)(*val);
  }

  fdevice = open(config.portname, O_RDWR | O_NONBLOCK);
  if (fdevice > 0) {
    if (setup_serialport(fdevice, config.baudrate, config.databits,
                         config.stopbit, config.parity) < 0) {
      close(fdevice);
      exit(EXIT_FAILURE);
    }
  } else {
  ...
  CTRL_INFO_t *ctrl_info = (CTRL_INFO_t *)malloc(sizeof(CTRL_INFO_t));
  ...
  printf("\33[42;37m %s %d %d,%c,%d  \33[0m\n", config.portname,
         config.baudrate, config.databits, config.parity, config.stopbit);
  printf("\33[42;37m Ctrl-R for Menu            \33[0m\n");

  pthread_t tid_reader, tid_input;
  pthread_create(&tid_reader, NULL, port_reader, ctrl_info);
  pthread_create(&tid_input, NULL, input_processor, ctrl_info);
